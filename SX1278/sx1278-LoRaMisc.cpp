...
uint32_t SX1278LoRaGetRFFrequency() {
  ...
  LoRaSettings.RFFrequency =
      ((uint32_t)SX1278LR->RegFrfMsb << 16) | ((uint32_t)SX1278LR->RegFrfMid << 8) | ((uint32_t)SX1278LR->RegFrfLsb);
...
void SX1278LoRaSetRFPower(int8_t power) {
  ...
  if ((SX1278LR->RegPaConfig & RFLR_PACONFIG_PASELECT_PABOOST) == RFLR_PACONFIG_PASELECT_PABOOST) {
    if ((SX1278LR->RegPaDac & 0x87) == 0x87) {
      ...
      SX1278LR->RegPaConfig =
          (SX1278LR->RegPaConfig & RFLR_PACONFIG_OUTPUTPOWER_MASK) | (uint8_t)((uint16_t)(power - 5) & 0x0F);
    } else {
      if (power < 2) {
        power = 2;
      }
      ...
      SX1278LR->RegPaConfig = (SX1278LR->RegPaConfig & RFLR_PACONFIG_MAX_POWER_MASK) | 0x70;
      SX1278LR->RegPaConfig =
          (SX1278LR->RegPaConfig & RFLR_PACONFIG_OUTPUTPOWER_MASK) | (uint8_t)((uint16_t)(power - 2) & 0x0F);
    }
  } else {
    if (power < -1) {
      power = -1;
    ...
    SX1278LR->RegPaConfig = (SX1278LR->RegPaConfig & RFLR_PACONFIG_MAX_POWER_MASK) | 0x70;
    SX1278LR->RegPaConfig =
        (SX1278LR->RegPaConfig & RFLR_PACONFIG_OUTPUTPOWER_MASK) | (uint8_t)((uint16_t)(power + 1) & 0x0F);
  }
...
void SX1278LoRaSetSymbTimeout(uint16_t value) {
  ...
  SX1278LR->RegModemConfig2 = (SX1278LR->RegModemConfig2 & RFLR_MODEMCONFIG2_SYMBTIMEOUTMSB_MASK)
